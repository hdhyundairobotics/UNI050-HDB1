        ...
        bcall(false)
{
  ...
  icp_matching_pose_sub_ = nh_.subscribe("/icp_scan_matcher/icp_matching_pose", 2, &AmclNode::icpMatchingPoseReceived, this);
  icp_alive_sub_ = nh_.subscribe("/icp_scan_matcher/icp_alive", 1, &AmclNode::icpCallBack, this);
...
void AmclNode::updatePoseFromServer()
{
  ...
  private_nh_.param("initial_cov_xx", init_cov_[0], 0.25);
  private_nh_.param("initial_cov_yy", init_cov_[1], 0.25);
  private_nh_.param("initial_cov_aa", init_cov_[2], 0.5);
  ...
  // ROS_WARN_STREAM("initial_pose_x: " << init_pose_[0]);
...
void
AmclNode::icpMatchingPoseReceived(const geometry_msgs::PoseWithCovarianceStampedConstPtr& msg)
{
  ROS_INFO("icpMatchingPose Received");
  handleInitialPoseMessage(*msg);
}

void
AmclNode::icpCallBack(const std_msgs::Bool& msg)
{
  bcall = msg.data;
}
...
AmclNode::applyInitialPose()
{
  ...
  // // service call for icp_scan_matcher
  // if (bcall == true)
  // {
  //   ros::Duration(1.0).sleep();

  //   ros::ServiceClient cli_srv_icp_call = nh_.serviceClient<std_srvs::SetBool>("/icp_scan_matcher/icp_call");
  //   ros::ServiceClient cli_srv_icp_matching = nh_.serviceClient<std_srvs::SetBool>("/icp_scan_matcher/icp_matching");
  //   std_srvs::SetBool srv_icp_call;
  //   std_srvs::SetBool srv_icp_matching;
  //   srv_icp_call.request.data = true;
  //   srv_icp_matching.request.data = true;

  //   cli_srv_icp_call.call(srv_icp_call);
  //   ros::Duration(3.0).sleep();
  //   cli_srv_icp_matching.call(srv_icp_matching);

  //   ros::Duration(3.0).sleep();
  //   srv_icp_call.request.data = false;
  //   cli_srv_icp_call.call(srv_icp_call);

  //   ROS_INFO("srv call from AMCL");
  // }
